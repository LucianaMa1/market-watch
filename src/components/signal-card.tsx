...
export function SignalCard({ signal }: { signal: Signal }) {
  ...
  return (
    <article className="rounded-3xl border border-white/10 bg-slate-950/70 p-6 shadow-lg shadow-black/20">
      ...
      {(signal.added.length > 0 || signal.removed.length > 0) && (
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-emerald-300">Added</p>
            <ul className="mt-2 space-y-2 text-sm text-slate-300">
              {signal.added.length ? signal.added.map((item) => <li key={item}>+ {item}</li>) : <li>No additions captured.</li>}
            </ul>
          </div>
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-rose-300">Removed</p>
            <ul className="mt-2 space-y-2 text-sm text-slate-300">
      ...
      <div className="mt-5 rounded-2xl border border-cyan-400/15 bg-cyan-400/5 p-4 text-sm leading-6 text-cyan-50">
